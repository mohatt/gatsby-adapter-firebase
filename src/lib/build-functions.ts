...
export const buildFunctions = async (
  ...
  const workspace: FunctionsWorkspace = { dir: outDir, files: new Map(), exports: [] }
  ...
  const writeVariantModuleFile = async (entry: FunctionEntry) => {
    const { variant, config, meta, modulePath, entryPath } = entry
    ...
    const factory: FunctionsRuntimeExport =
      variant === 'cached' ? 'createCachedHttpsFunction' : 'createHttpsFunction'
    const lines = [
  ...
  const createVariant = async (
    ...
    const variantEntry: FunctionEntry = {
      id,
      variant,
      deployId,
      modulePath,
      ...
      meta: {
        id,
        name: name === 'SSR & DSG' ? (variant == 'cached' ? 'DSG' : 'SSR') : name,
    ...
    await writeVariantModuleFile(variantEntry)
    await addWorkspaceFile(modulePath)
    workspace.exports.push(variantEntry)
  ...
  for (const func of functionsManifest) {
    ...
    try {
      const defaultVariant = await createVariant(id, name, entryToPath, files, 'default')
      // Create a cached variant if the function is cached
      const cachedVariant = cachedIds.has(id)
        ? await createVariant(`${id}-cached`, name, entryToPath, files, 'cached')
        : undefined
  ...
  for (const { deployId, modulePath } of workspace.exports) {
  ...
  await Promise.all(
    workspace.exports.map(async (entry) => {
      ...
      await writeVariantModuleFile(entry)
    }),
  )
